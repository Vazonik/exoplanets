  ...
  render() {
    let foundRender = (
      <div style={{ marginTop: 10 }}>
        Wpisz nazwę obiektu i kliknij przycisk, bo go wyszukać.
      </div>
    );
    if (!this.state.found) {
      foundRender = (
      ...
      foundRender = (
        <FoundPlanet
          ...
          data={this.state.found.content}
          type={this.state.found.type}
        />
      );
    }
    ...
    return (
      <>
        ...
        <PageCanvas>
          <SearchField
            exoplanets={this.props.exoplanets}
            value={this.props.value}
            searchCallback={this.handleSearch}
          />
          {foundRender}
          <div style={{ color: "#ffffff50" }}>
            Ta sekcja strony będzie szczególnie rozwijana w przyszłości.
          </div>
        </PageCanvas>
      </>
    );
